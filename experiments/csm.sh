...
# ################################################################
beforemigr=$(date +%s%N)
sshroot $nodedst "csm_dest_nocp.sh $basenet$src trafficgen user1 https://$basenet$dst:7000 $basenet$dst https://$basenet$dst '-v /etc/timezone:/etc/timezone:ro -v /etc/localtime:/etc/localtime:ro -p 8080:8080 --name trafficgen';"
aftermigr=$(date +%s%N)
