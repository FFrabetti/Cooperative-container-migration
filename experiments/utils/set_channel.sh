...
function setValues {
	...
	shift
	local to=0
	sshroot ${hname[$from]} "sudo tc qdisc del dev $ip_if root; tc qdisc add dev $ip_if handle 1: root htb; tc class add dev $ip_if parent 1: classid 1:1 htb rate 900Mbps ceil 900Mbps;"
	while (( $# )); do
...
function setUniValues {
	if [ $3 != "0" ]; then
		echo "${hname[$1]} -> ${hname[$2]} delay=$3ms, bw=$4mbit"
		local to=$2
		sshroot ${hname[$1]} "tc class add dev $ip_if parent 1:1 classid 1:$((to+11)) htb rate $4mbit;
