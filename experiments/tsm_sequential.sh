...
# bash tsm.sh (channelparams | '0') loadparams loadtimeout layersize appversion respsize volumesize
prevchannel=""
i=0
starti=0
if [ $# -eq 1 ]; then
	...
	for vs in "100" "1MB" "10MB" "100MB"; do
		for ld in args/srchigh_load.txt args/dsthigh_load.txt args/eqmed_load.txt; do
			...
			if [ -f $ld ] && [ $i -ge $starti ]; then
				...
				echo -e "tsm.sh $charg $ld $loadtimeout $ls app$ls $respsize $vs \n----------------"
				bash tsm.sh $charg $ld $loadtimeout $ls "app$ls" $respsize $vs 2>&1 | tee "tsm${i}.out"
				sleep 10
