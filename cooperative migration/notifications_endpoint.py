...
class Server(BaseHTTPRequestHandler):
  ...
  def respond(self):
    ...
    if debug_mode:     # debug
        sec = int(time.time())
        with open(str(sec) + ".json", "w") as write_file:
            json.dump(data, write_file)

    if "events" in data:
        for event in data["events"]:
            if event["action"] == "push" and event["target"]["mediaType"] == "application/octet-stream":
                url = event["target"]["url"]
                index = url.rindex("/")
                print('url=%s,digest=%s' % (url[0:index], url[index+1:]))
                sys.stdout.flush()
